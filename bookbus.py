...
def show_bus():
    ...
    Label(frame2,text="Select Bus",fg="green",font=('Arial 10 bold')).grid(row=4,column=1,padx=10)
    
    Label(frame2,text="Operator",fg="green",font=('Arial 10 bold')).grid(row=4,column=2,padx=10)
    
    Label(frame2,text="Bus Type",fg="green",font=('Arial 10 bold')).grid(row=4,column=3,padx=10)

    Label(frame2,text="Available/Capacity",fg="green",font=('Arial 10 bold')).grid(row=4,column=4,padx=10)

    Label(frame2,text="Fare",fg="green",font=('Arial 10 bold')).grid(row=4,column=5,padx=10)
...
def display_buses(buses_running):
    ...
    for i in range(len(buses_running)):
        ...
        if int(available)>0:
            ...
            Radiobutton(frame2, text=f'Bus {i + 1}', bg="light blue", font=('Arial 10 bold'),
            ...
            l = l + 1
            Label(frame2, text=operator_name[0], fg="blue", font=('Arial 10 bold')).grid(row=k, column=l, padx=10)
            bus_type = bus_booking_system.show_bus_type(bus_id)
            l = l + 1
            Label(frame2, text=bus_type[0], fg="blue", font=('Arial 10 bold')).grid(row=k, column=l, padx=10)
            available = bus_booking_system.show_avail(bus_id,Journey_date)
            cap = bus_booking_system.show_cap(bus_id)
            l = l + 1
            Label(frame2, text=f'{available}/{cap[0]}', fg="blue", font=('Arial 10 bold')).grid(row=k, column=l,
                                                                                                           padx=10)
            l = l + 1
            fare = bus_booking_system.show_fare(bus_id)
            Label(frame2, text=fare[0], fg="blue", font=('Arial 10 bold')).grid(row=k, column=l, padx=10)
            k = k + 1
            l = 1
    
    Button(frame2, text="Proceed to Book", bg="red", font=('Arial 10 bold'), command=book_bus).grid(row=k-1, column=7)
...
frame.pack()
image = PhotoImage(file="Mybus1.png")
...
title_project= Label(frame2,text="Online Bus Booking System",relief="ridge",bg="light blue",fg="red",font=('Arial 42 bold'))
title_project.grid(row=0,column=0,pady=20,columnspan=20)

title_projexts 

Label(frame2,text="Enter Journey Details",bg="green",font=('Arial 15')).grid(row=1,column=0,pady=20,columnspan=20)

Label(frame2,text="From:",font=('Arial 10 bold')).grid(row=3,column=0)

start=Entry(frame2)
start.grid(row=3,column=1)
Label(frame2,text="To:",font=('Arial 10 bold')).grid(row=3,column=2,padx=10)
finish=Entry(frame2)
finish.grid(row=3,column=3)

Label(frame2,text='''Journey Date:(DD/MM/YYYY)''',font=('Arial 10 bold')).grid(row=3,column=4,padx=10)
date=Entry(frame2)
date.grid(row=3,column=5)
#show bus button
Button(frame2,text="Show Bus",bg="green",font=('Arial 10 bold'),command=show_bus).grid(row=3,column=6,padx=10)
# home button
